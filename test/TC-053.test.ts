...
/**
 * TC-053 (TC-B1): ETF gain → Bucket A
 *
 * BUY  10 IE00B4L5Y983 @ EUR 100/unit (totalEUR=1000), feesEUR=0
 * SELL 10 IE00B4L5Y983 @ EUR 140/unit (totalEUR=1400), feesEUR=0
 * gainLossEUR = 400
 *
 * IE00B4L5Y983 is an ETF (bucketGain="A"), so lot.bucket = "A"
 * bucketA.groups[0].taxRate = 0.26, plusvalenze = 400, imposta = 104.00
 * bucketB.plusvalenze = 0
 */
...
describe("TC-053 (TC-B1): ETF gain → Bucket A", () => {
  ...
  it("bucketA.groups has one entry", () => {
    expect(report.bucketA!.groups).toHaveLength(1);
  });
  ...
  it("bucketA.groups[0].imposta === 104.00", () => {
    expect(report.bucketA!.groups[0].imposta).toBe(104.0);
  });
  ...
  it("report.plusvalenze === 400", () => {
    expect(report.plusvalenze).toBe(400);
  });
});
